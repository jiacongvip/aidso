...
import { X, Trash2, Plus, Sparkles, FolderUp, PenTool, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const AddTrackingModal = ({ isOpen, onClose, projectId, onSaved }: AddTrackingModalProps) => {
  ...
  const handleConfirm = async () => {
    ...
    setSuccess('');
    try {
      const { res, data } = await apiJson(`/api/monitoring/projects/${projectId}/works`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rows: normalizedRows }),
      });
      if (!res.ok) {
        setError(apiErrorToMessage(data, `保存失败（HTTP ${res.status}）`));
        return;
      }

      setSuccess(`已添加 ${Array.isArray((data as any)?.created) ? (data as any).created.length : normalizedRows.length} 条`);
      if (onSaved) onSaved();
      setTimeout(() => {
        onClose();
        setSuccess('');
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-xl shadow-2xl animate-scale-in flex flex-col max-h-[90vh] overflow-hidden">
        ...
        {(error || success) && (
          <div className="px-8">
            {error && (
              <div className="mb-3 p-3 bg-red-50 border border-red-100 rounded-lg text-red-600 text-xs flex items-center gap-2">
                <AlertCircle size={14} />
                {error}
              </div>
            )}
            {success && (
              <div className="mb-3 p-3 bg-green-50 border border-green-100 rounded-lg text-green-700 text-xs flex items-center gap-2">
          ...
          <div className="flex border border-gray-200 rounded-lg overflow-hidden p-1 bg-gray-50">
            <button
              ...
              className={`flex-1 py-2 rounded-md text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                activeTab === 'manual' ? 'bg-[#7c3aed] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-md text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                activeTab === 'upload' ? 'bg-[#7c3aed] text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-4 scrollbar-hide">
          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-[#7c3aed] cursor-pointer hover:underline font-medium">
              <Sparkles size={14} /> 使用AI通过作品链接解析标题（可后续做）
            </div>
        ...
        <div className="p-6 text-center border-t border-gray-50 bg-gray-50/30">
          <button
            ...
            disabled={saving}
            className="bg-[#7c3aed] text-white px-12 py-2.5 rounded-lg text-sm font-bold hover:bg-[#6d28d9] transition-all shadow-lg shadow-purple-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {saving ? '保存中...' : '确 认'}
          </button>
        </div>
